...
class Ai_ssd_without_area(object):
    def __init__(self):
        self.is_init = False

    def _whole_result(self,detections):
        ...
        for i in range(len(top_indices)): #top_conf.shape[0]):
    ...
    def get_running_rec(self,rec):
        ...
        running_rec['img'] = self.preprocess_img(img,running_rec['img_coords'])
        return running_rec

    def get_splits_rec(self,rec):
        #c
        #rec['img_coords'] = rawImg.getbbox()
        #logger.debug(running_rec['img_coords'])
        #logger.debug(running_rec['area_coords'])
        x,y,w,h = rec['img_coords']

        
        #logger.debug("The picture format:{}".format(rawImg.format_description))
        left_box = [x,y,h,h]
        right_box = [w-h,y,w,h]
        # 如果长宽异常
        if w<h:
        ...
        logger.debug("left_box:{}".format(left_box))
        rec['sp1'] = self.preprocess_img(rec['img'],left_box)
        logger.debug("right_box:{}".format(right_box))
        rec['sp2'] = self.preprocess_img(rec['img'],right_box)
        return rec
    ...
    def preprocess_img(self,img,coords):
        """
        ...
        logger.debug("shape:{},decode picture content:{}".format(img.shape,img[:3,1,1]))

        if img.ndim == 2:
            recog_img = img[:, :, np.newaxis]
            if color:
                recog_img = np.tile(recog_img, (1, 1, 3))
        elif img.shape[2] == 4:
            recog_img = img[:, :, :3]

        recog_img = img[ymin:ymax, xmin:xmax, :]

        logger.debug("coords:%s"%(coords,))
        logger.debug("the recorg picture shape: {}".format(recog_img.shape))
        return recog_img
   ...
   #     return images
    ...
    def pred_one_pic(self,rec):
        ...
        ####################################################
        splits =2 
        self.net.blobs['data'].reshape(splits,3,self.image_resize,self.image_resize)
        rec = self.get_splits_rec(rec)

        transformed_image = self.transformer.preprocess('data', rec['sp1'])
        self.net.blobs['data'].data[0,...] = transformed_image
        transformed_image = self.transformer.preprocess('data', rec['sp2'])
        ...
        del rec['sp1']
        del rec['sp2']
        return self._whole_result(detections)
    ...
    def pred_picture(self, records, ai_types =[1] ):
        ...
        for i in range(img_num):
            r = self.pred_one_pic(running_recs[i])
